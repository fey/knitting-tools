 ...
 *
 * Здесь нет кнопки «развернуть»: разворачивать нечего, страница и так несёт схему,
 * зум и плашку прогресса. В шторку уехали только ручки расчёта, и вызывают её кнопкой
 * «Настройки» в верхней полосе — на телефоне она наезжает сверху, на широком экране
 * выезжает слева, на месте той самой левой колонки.
 *
 * На странице остались вводка и «Итог». Вводка объясняет инструмент, а не расчёт,
 * и прячется своей же кнопкой (§6.1); «Итог» несёт предупреждения о покрытии и длине
 * мыска, а спрятанное предупреждение не предупреждает.
 *
...
<template>
  <div class="min-h-screen bg-white pt-9 text-slate-900">
    ...
    <main class="mx-auto flex max-w-[1400px] flex-col gap-4 px-3 pt-3 pb-56">
      <!-- Вводка остаётся на странице, а не уезжает в шторку: она объясняет сам
           инструмент, а шторка — про ручки расчёта. Прячется она своей же кнопкой
           (§6.1), и на десятом заходе от неё остаётся одна строка над схемой. -->
      <IntroNote />
      <ToeChart />
