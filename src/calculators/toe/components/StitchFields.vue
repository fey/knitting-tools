 ...
 *
 * «И только ими» сказано **про закрытия**, а не про всё содержимое подписи: запрет в том,
 * чтобы не пересказывать здесь конструкцию мыска. Вторая фраза подписи — про счёт в круге
 * (ниже) — тикету #15 не противоречит и добавлена §4 осознанно.
 *
 ...
 *
 * **Подсказки зовут вписать своё число (§4).** Клавиатурный ввод не был ограничен никогда,
 * но на экране об этом не говорилось: кнопки подписаны «−4»/«+4», подсказка у конечных
 * перечисляла «шаг 4: 16, 20, 24», — и поле читалось набором допустимых значений, а не полем.
 * Цена была прямая: 62 начальных петли кнопками недостижимы вовсе, а мысок такой существует.
 * Обещание честное — поле принимает любое чётное, а несходящееся подтягивает нормализатор
 * и говорит, куда подтянул; новой свободы подсказка не выдаёт.
 *
 * **Конечные петли считаются в круге, и подпись это называет.** Ниже стоит схема, на которой
 * показана одна половина (§7): «11 петель на закрытие» легко имеется в виду с одной стороны,
 * а поле ждёт 22. Спутать эти два числа нечем, кроме подписи, — живого пересчёта половины
 * на экране нет.
 *
...
<template>
  <section class="rounded border border-slate-200 p-4" data-testid="stitch-fields">
    ...
    <div class="mt-3">
      ...
      <p class="mt-1 text-sm text-slate-500" data-testid="initial-hint">
    ...
    <div class="mt-4">
      ...
      <p class="mt-1 text-sm text-slate-500" data-testid="final-hint">
        ...
        >
        · обычно 16–24 · своё число вписывается так же
      </p>
