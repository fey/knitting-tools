...
/**
 * Кнопка «Поделиться» (§10.5, тикет #8). Отдельный корень страницы (см.
 * `main.ts`) — с `App.vue` не пересекается: соседняя ветка сейчас правит блоки
 * оболочки, а спека места для кнопки не называет.
 *
 ...
 *
 * Кнопка **зафиксирована (`position: fixed`)**, а не стоит в потоке страницы —
 * и это тоже часть гарантии «не роняем недонабранное». Подпись починки (§9.2)
 * вставляется в DOM тем же синхронным `blur`, что чинит поле, — если бы кнопка
 * стояла ниже поля в обычном потоке, эта вставка сдвинула бы её вниз между
 * `mousedown` и `mouseup` одного и того же клика, и клик промахнулся бы мимо
 * (проверено: без `fixed` тест на блюр-починку падает на пустом `__shared`).
 */
...
<template>
  <button
    type="button"
    class="fixed top-3 right-3 z-20 rounded-full border border-slate-300 bg-white px-4 py-2 text-sm font-medium text-slate-900 shadow-sm"
