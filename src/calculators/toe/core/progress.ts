...
/**
 * Сообщение по завершении мыска. Плашка прибита к низу экрана и стоит там всегда —
 * отдельного состояния «выключено» у неё нет (§8), поэтому у завершения тоже есть текст.
 */
export function toeDoneText(calculation: Pick<ToeCalculation, 'finalReal'>): string {
  return `Мысок связан — закрывай ${calculation.finalReal} петель трикотажным швом`
}
